...
export function AdvancedMetricCard({
  ...
  const renderChart = () => {
    // Generate a flat placeholder line when no real data is available
    const effectiveData = (!chartData || chartData.length < 2)
    ...
    const maxValue = Math.max(...effectiveData.map(d => d.value))
    const minValue = Math.min(...effectiveData.map(d => d.value))
    ...
    const points = effectiveData.map((point, index) => ({
      x: padding + (index / (effectiveData.length - 1)) * (width - padding * 2),
