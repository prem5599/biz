...
export class CurrencyConverter {
  ...
  /**
   * Fetches exchange rates from our own backend API (which caches them server-side).
   * No direct calls to external services from the browser.
   */
  async updateExchangeRates(): Promise<void> {
    try {
      const { currencyApi } = await import('./api')
      const response = await currencyApi.getRates()
      const rates = response.data?.data?.rates as Record<string, number>
  ...
  private setFallbackRates(): void {
    const fallbackRates: Record<string, number> = {
      USD: 1, EUR: 0.85, GBP: 0.73, JPY: 110, CNY: 6.4,
      INR: 83, AUD: 1.35, CAD: 1.25, SGD: 1.35, AED: 3.67,
      SAR: 3.75, BRL: 5.2, MXN: 18, ZAR: 15,
    }
  ...
  async convertCurrency(
    ...
    // Update rates if stale (older than 1 hour)
