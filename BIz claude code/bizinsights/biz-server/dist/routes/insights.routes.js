    ...
    try {
        ...
        if (dataPoints.length > 0) {
            ...
            insights.push({
                ...
                description: 'No data has been synced in the last 30 days. Make sure your integrations are connected and syncing properly.',
                impactScore: 80,
                isRead: false,
                metadata: {},
            });
        ...
        for (const insight of insights) {
            await prisma_1.prisma.insight.upsert({
                where: { id: insight.id },
                create: insight,
                update: insight,
            });
...
router.get('/trending', auth_middleware_1.authenticate, async (req, res) => {
    try {
...
router.post('/:id/read', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const insight = await prisma_1.prisma.insight.update({
            where: { id },
            data: { isRead: true },
        });
...
router.post('/feedback', auth_middleware_1.authenticate, async (req, res) => {
    try {
...
router.delete('/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.prisma.insight.delete({ where: { id } });
